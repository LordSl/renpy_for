...
for i in range(0, len(char_list)):
    file_rpy.write(
        "image " + char_list[i] + "_resized" + " = im.Scale(\"" + char_list[i] + ".png\"" + ", " + char_x[i] + "," +
        char_y[i] + ")\n")
file_rpy.write("\n")
        ...
        if line != char_tag:  # which means line is a content line
            ...
            #####PC and KP
                ...
                if char_tag != previous_char_tag:
                    file_rpy.write(
                        "    show " + char_tag + "_resized" + " at " + location_list[char_list.index(char_tag)] + "\n")
                ####then deal with dialog
                file_name_voice = char_tag + "/" + str(global_voice_num).zfill(
                    3) + char_tag + " " + line + ".mp3"  # 语音文件的路径
